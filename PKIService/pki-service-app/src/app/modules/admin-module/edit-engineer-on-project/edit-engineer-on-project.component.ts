...
@Component({
  selector: 'app-edit-engineer-on-project',
  templateUrl: './edit-engineer-on-project.component.html',
  styleUrls: ['./edit-engineer-on-project.component.css']
})
export class EditEngineerOnProjectComponent  implements OnInit{
  ...
  updateProjectWork(){
    ...
    let startDate = this.updateProjectWorkForm.get("startDate")?.value
    let endDate = this.updateProjectWorkForm.get("endDate")?.value
    ...
    if(this.isValidDate && this.isValidStart && this.isValidEnd && this.isSelected){
      this.projectWorkService.updateProjectWork(projectWork).subscribe(res=>{
        console.log(res);
        this.router.navigate(['projects', this.projectID, 'employees']);
      }, error => 
      {
